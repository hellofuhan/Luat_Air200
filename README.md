...
\script是合宙范例LUA脚本，whole_project 是一款已经商用的GPS定位器，可以下到合宙小蛮GPS定位器中运行。合宙EVB开发板对应的硬件参考和AT命令位于\doc\Air200模块相关文档。 timer、 UART 等是针对各单个应用或功能的示例代码。 lib 是所有用户脚本都需要用到的库文件，这些库文件将经常使用的AT命令以函数形式封装，方便用户使用。
